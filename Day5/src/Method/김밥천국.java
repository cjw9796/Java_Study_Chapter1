...
public class 김밥천국 {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 
		 * main 메소드 main 메소드는 프로그램의 시작점 역할을 하는 JAVA의 약속된 내용이다. 즉, main 메소드가 없는 프로그램은 별도로
		 * 동작할 수가 없다. 컴퓨터가 소스 코드를 읽을 때 main 메소드를 실행하고 거기에 정의된 로직에 따라 프로그램이 동작하게 된다.
		 * 
		 * 메소드리턴 타입 
		 * 리턴 타입 위에서 설명하였듯이 메소드의 리턴 타입은 자료형 또는 void가 올 수 있다. 자료형이 리턴 타입으로 오면
		 * 반환(return)해 주는 값(메소드 자료형과 동일한 값)이 반드시 있어야 하고 void는 반환 값이 없다. 
		 * 다르게 말하면 반환하고자 하는 데이터가 있다면 그 데이터의 자료형을 메소드의 자료형으로 적어줘야 한다. 
		 * 
		 * 
		 * 리턴(return) 문 
		 * 메소드 내에서 동작하고 얻은 결과(필요한 데이터)
		 * 를 반환해주는 역할을 한다. return문 뒤에는 어떠한 구문도 올 수 없다.(컴파일 에러 발생) 
		 * 다르게 말하면 return문을 사용하면 값을 반환해 주고 해당 메소드는 실행이 종료된다.
		 * 
		 * 
		 * 
		 */
